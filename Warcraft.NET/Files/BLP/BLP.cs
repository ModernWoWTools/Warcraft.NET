...
namespace Warcraft.NET.Files.BLP
{
    ...
    public class BLP
    {
        ...
        public BLP(Image<Rgba32> image, BLPPixelFormat pixelFormat)
        {
            ...
            Header = new BLPHeader
            {
                CompressionType = TextureCompressionType.DXTC,
                AlphaBitDepth = 8,
        ...
        private Image<Rgba32> DecompressMipMap(byte[] inData, uint mipLevel)
        {
            ...
            switch (Header.CompressionType)
            {
                ...
                case TextureCompressionType.DXTC:
                    {
                        var squishOptions = SquishFlags.DXT1;
                        if (Header.PixelFormat == BLPPixelFormat.DXT3)
                        {
                            squishOptions = SquishFlags.DXT3;
                        }
                        else if (Header.PixelFormat == BLPPixelFormat.DXT5)
                        {
                            squishOptions = SquishFlags.DXT5;
                        }

                        map = Squish.DecompressToImage(inData, (int)targetXRes, (int)targetYRes, squishOptions);
                        break;
                    }
        ...
        /// The number of mipmaps returned will be <see cref="Warcraft.NET.Files.BLP.BLP.GetNumReasonableMipMapLevels"/> + 1.
        /// </summary>
        ...
        private byte[] CompressImage(Image<Rgba32> inImage, uint mipLevel)
        {
            ...
            using (var resizedImage = ResizeImage(inImage, (int)targetXRes, (int)targetYRes))
            {
                ...
                else if (Header.CompressionType == TextureCompressionType.DXTC)
                {
                    ...
                    var dest = new byte[Squish.GetStorageRequirements((int)targetXRes, (int)targetYRes, squishOptions)];
                    Squish.CompressImage(rgbaBytes, (int)targetXRes, (int)targetYRes, dest, squishOptions);
                    colourData = new List<byte>(dest);
                }
